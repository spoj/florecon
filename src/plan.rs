...
///
/// v7 makes `Report` allocation-native: `allocations` are the single source of
/// truth and row/group views are explicit client projections.
...
pub enum Plan {
    ...
    /// Enter lot mode for `inner`: initialize each row's `original` and current
    /// residual lot amount from `amount`, then let `seq` thread shrinking
    /// residuals through later steps. `partition` outside this node remains the
    /// hard reach boundary for those residual lots.
    Lots { amount: ScalarRef, inner: Box<Plan> },
    ...
    /// Consume small residual lots into variance/writeoff/unmatched classes.
    /// If `by` is present, all consumed lots sharing that scalar key become one
    /// bucketed group; otherwise each consumed lot becomes its own singleton
    /// group. `max_bps` compares `abs(current residual)` to `abs(original lot)`;
    /// `max_abs` is an absolute minor-unit threshold. At least one threshold
    /// should be set for useful plans.
    SoakSmall {
    ...
    /// This is normally a terminal classifier after more selective soakers.
    SoakAll {
        origin: String,
        #[cfg_attr(feature = "serde", serde(default))]
        by: Option<ScalarRef>,
...
impl Session {
    ...
    fn run_strategy(
        ...
        let bag: Vec<Item<LoweredRow>> = self
            .rows
            .iter()
            .map(|(id, row)| Item::row(*id, row.clone()))
            .collect();
        let input = bag.len();
...
impl<E: Clone> Recon<E> {
    ...
    pub fn solve(&mut self) -> Result<(), ApiError> {
        ...
        let mut frozen_whole_rows = BTreeSet::new();
        let mut frozen_lots: BTreeMap<ExtId, (i64, i64)> = BTreeMap::new(); // id -> (frozen amount, original)
        for g in self.groups.iter().filter(|g| g.is_frozen()) {
            for a in &g.allocations {
                if a.lot {
                    let e = frozen_lots.entry(a.id).or_insert((0, a.original));
                    e.0 += a.amount;
                    e.1 = a.original;
                } else {
                    frozen_whole_rows.insert(a.id);
                }
            }
        }
        let bag: Vec<Item<E>> = self
            ...
            .iter()
            .filter_map(|(id, item)| {
                if let Some((frozen, original)) = frozen_lots.get(id).copied() {
                    let rem = original - frozen;
                    (rem != 0).then(|| Item {
                        id: *id,
                        original,
                        amount: rem,
                ...
                } else if frozen_whole_rows.contains(id) {
                    None
                } else {
                    Some(Item::row(*id, item.clone()))
                }
            })
            .collect();
        let meta: BTreeMap<ExtId, (i64, bool)> =
            bag.iter().map(|i| (i.id, (i.original, i.lot))).collect();
        ...
        for g in new_groups {
            self.groups.push(GroupRec {
                ...
                allocations: g
                    ...
                    .into_iter()
                    .map(|a| {
                        let (original, lot) = meta.get(&a.id).copied().unwrap_or((0, false));
                        StoredAlloc {
                            id: a.id,
                            amount: a.amount,
                            original,
    ...
    pub fn ungroup(&mut self, ids: &[ExtId]) -> Result<(), ApiError> {
        ...
        // preserving split-lot amounts. If an id had no live allocation yet,
        // fall back to a row-mode singleton.
...
mod tests {
    ...
    fn plan_json_round_trips() {
        ...
        assert_eq!(rep.strict_assignments().unwrap().len(), 2);
    }
    ...
    fn ws_conserves(ws: &Workspace) {
        let rep = ws.report();
        // This strict whole-row plan can still be projected to one group per row.
        let n = rep.strict_assignments().unwrap().len();
        assert_eq!(n, ws.len(), "strict projection covers every row");
    }
    ...
    fn workspace_remove_keeps_conservation() {
        ...
        assert_eq!(
            rep.strict_assignments().unwrap(),
            vec![(1, rep.groups[0].group_id)]
        );
        ws_conserves(&ws);
    }
    ...
    fn workspace_manual_group_and_ungroup() {
        ...
        let rep = ws.report();
        let assignments = rep.strict_assignments().unwrap();
    ...
    fn warm_partition(rep: &WorkspaceReport) -> BTreeSet<Vec<ExtId>> {
        let multi: Vec<u64> = rep
            ...
            .map(|g| g.group_id)
            .collect();
        report_partition(&rep.strict_assignments().unwrap(), &multi)
    }
    ...
    fn run_equiv(plan: Plan, shard_of: impl Fn(u64) -> i64, seed: u64) {
        ...
        for _ in 0..300 {
            ...
            let rep = ws.report();
            assert_eq!(
                rep.strict_assignments().unwrap().len(),
                ws.len(),
                "conservation"
            );
    ...
    fn warm_flow_objective_stable_under_contention() {
        ...
        for _ in 0..200 {
            match rng.below(4) {
                ...
                2 => {
                    let rep = ws.report();
                    let singleton_gids: BTreeSet<u64> = rep
                        .groups
                        .iter()
                        .filter(|g| g.size == 1 && g.status == Status::Live)
                        .map(|g| g.group_id)
                    ...
                    let singles: Vec<ExtId> = rep
                        .allocations
                        .iter()
                        .filter(|a| singleton_gids.contains(&a.group_id))
                        .map(|a| a.id)
                        .take(2)
                        .collect();
            ...
            ws.solve().unwrap();
            let covered: BTreeSet<ExtId> = ws.report().allocations.iter().map(|a| a.id).collect();
            assert_eq!(covered.len(), ws.len());
        }
    }
    ...
    fn soak_small_classifies_by_residual_vs_original() {
        ...
        let mut s = Session::new(schema);
        // 1% remainder: +1000 matches -990, +10 gets soaked into class 42.
        ...
        let variance = rep
            ...
            .unwrap();
        assert_eq!(variance.net, 10);
        assert!(rep.groups.iter().all(|g| g.origin != "unmatched"));
    }

    #[test]
    fn workspace_group_allocations_takes_exact_live_amounts() {
        let plan = Plan::Lots {
            amount: "usd".into(),
            inner: Box::new(Plan::Exact {
                amount: "usd".into(),
        ...
        let mut ws = Workspace::new(schema(), plan).unwrap();
        ws.upsert(1, row(100, 0, 0, 100, &[])).unwrap();
        ws.upsert(2, row(-40, 0, 0, -40, &[])).unwrap();
        ws.solve().unwrap();

        let gid = ws
            .group_allocations(
                &[
                    AllocationSpec { id: 1, amount: 40 },
                    AllocationSpec { id: 2, amount: -40 },
                ],
                "manual_alloc",
            )
            .unwrap();
        let rep = ws.report();
        let manual = rep.groups.iter().find(|g| g.group_id == gid).unwrap();
        assert_eq!(manual.origin, "manual_alloc");
        assert_eq!(manual.status, Status::Frozen);
        assert_eq!(manual.net, 0);
        assert!(
            rep.allocations
                .iter()
                .any(|a| a.group_id == gid && a.id == 1 && a.amount == 40)
        );
        assert!(
            rep.allocations
                .iter()
                .any(|a| a.group_id == gid && a.id == 2 && a.amount == -40)
        );
        assert!(rep.allocations.iter().any(|a| a.id == 1 && a.amount == 60));
    }

    #[test]
    fn workspace_remove_allocations_targets_one_group() {
        let plan = Plan::Lots {
            amount: "usd".into(),
            inner: Box::new(Plan::Exact {
                amount: "usd".into(),
        ...
        let mut ws = Workspace::new(schema(), plan).unwrap();
        ws.upsert(1, row(100, 0, 0, 100, &[])).unwrap();
        ws.upsert(2, row(-100, 0, 0, -100, &[])).unwrap();
        ws.solve().unwrap();
        let group_id = ws
            .report()
            .groups
            .iter()
            .find(|g| g.origin == "exact_1to1")
            .unwrap()
            .group_id;
        ws.remove_allocations(group_id, &[1]).unwrap();
        let rep = ws.report();
        assert!(!rep.groups.iter().any(|g| g.group_id == group_id));
        assert!(rep.allocations.iter().any(|a| a.id == 1 && a.amount == 100));
        assert!(
            rep.allocations
                .iter()
                .any(|a| a.id == 2 && a.amount == -100)
        );
    }
}
